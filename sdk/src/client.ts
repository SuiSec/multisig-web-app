...
export class SagatClient {
	...
	/** Rename a multisig (relay-side metadata only). */
	async renameMultisig(address: string, name: string) {
	...
	async recordPackage(data: RecordPackageRequest) {
		return this.#request<PackageRecord>('/packages', {
			method: 'POST',
			body: JSON.stringify(data),
		});
	}
	...
	async getPackageRecords(
		...
		const q = network
			...
			: '';
		return this.#request<PackageRecord[]>(
			`/packages/${encodeURIComponent(packageId)}${q}`,
		);
	}
	...
	/** Public: every package governed by a multisig. */
	async listMultisigPackages(
		address: string,
		network?: string,
	) {
		const q = network
			...
			: '';
		return this.#request<PackageRecord[]>(
			`/packages/by-multisig/${encodeURIComponent(address)}${q}`,
		);
	}
	...
	/**
	 * Call this to cast a reject vote on a proposal.
	 * @param proposalId
	 * @param data Expects a signed `PersonalMessages.rejectProposal()` message.
	 * @returns `{ unreachable }` — true once rejects make the threshold unreachable.
	 */
	async rejectProposal(
		proposalId: number,
		data: RejectProposalRequest,
	) {
		return this.#request<RejectProposalResponse>(
	...
	// Draft multisig endpoints (members collected by address before finalize).
	async createDraft(
		data: CreateDraftRequest,
	): Promise<CreateDraftResponse> {
		return this.#request<CreateDraftResponse>('/drafts', {
			method: 'POST',
			body: JSON.stringify(data),
		});
	}
	...
	async getDraft(id: string): Promise<DraftView> {
		return this.#request<DraftView>(
			`/drafts/${encodeURIComponent(id)}`,
		);
	}

	async joinDraft(id: string): Promise<JoinDraftResponse> {
		return this.#request<JoinDraftResponse>(
			`/drafts/${encodeURIComponent(id)}/join`,
			{ method: 'POST' },
		);
	}

	async listDrafts(): Promise<DraftView[]> {
		return this.#request<DraftView[]>('/drafts');
	}

	async cancelDraft(id: string) {
		return this.#request<{ success: boolean }>(
			`/drafts/${encodeURIComponent(id)}/cancel`,
			{ method: 'POST' },
		);
	}
